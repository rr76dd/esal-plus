...
export default function CreateClient() {
  ...
  const handleSubmit = async () => {
    try {
      ...
      // التحقق من وجود المستخدم قبل إنشاء العميل
      if (!auth.currentUser) {
      ...
      // إضافة العميل الجديد
      const customerId = await customerServices.createCustomer({
      ...
      if (!customerId) {
        throw new Error('لم يتم إنشاء العميل بشكل صحيح');
      }
    ...
    <div className="container mx-auto p-6 space-y-6">
      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center mb-8 gap-4 pt-6 sm:pt-4">
        <div className="flex items-center">
          ...
          <h1 className="text-3xl font-bold mr-2">إضافة عميل جديد</h1>
        </div>
      </div>
